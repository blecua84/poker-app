 ...
 * @author blecua84
 */
public class TexasHoldEmTablePanel extends JPanel implements Strategy {
    ...
    private static final Point[] COMMUNITY_CARDS_POSITIONS = {
            new Point(478, 348), new Point(538, 348), new Point(598, 348),
            new Point(658, 348), new Point(718, 348)
    };

    private static final Point[] PLAYER_POSITIONS
            = {new Point(1045, 333), new Point(999, 522), new Point(735, 606),
            new Point(445, 606), new Point(181, 522), new Point(135, 333),
            new Point(181, 144), new Point(445, 60), new Point(735, 60), new Point(999, 144)
    };

    private static final Point[] PLAYER_BET_POSITIONS = {new Point(945, 388), new Point(900, 506),
            new Point(739, 574), new Point(449, 574), new Point(285, 506),
            new Point(241, 388), new Point(285, 270), new Point(449, 202),
            new Point(739, 202), new Point(900, 270)
    };
    ...
    public void setStrategy(Strategy delegate) {
    ...
    private void paintPlayer(Graphics2D g2, int i) {
        ...
        if (players[i] != null) {
            ...
            if (bets[i] != null || players[i].getChips() > 0) {
                ...
                if (players[i].isActive() || players[i].getState() == PlayerState.ALL_IN) {
    ...
    public synchronized void check(List<Card> communityCards) {
        ...
        for (int i = 0; i < bets.length; i++) {
            if (bets[i] != null && bets[i] != BetCommandType.ALL_IN) {
                bets[i] = null;
            }
        }
        ...
        if (playersByName.isEmpty()) {
            ...
            for (i = 0; i < players.length; i++) {
                if (players[i] != null && !currentPlayers.contains(players[i].getName())) {
                    ...
                    players[i].setState(PlayerState.OUT);
                }
            }
        }
        pots = new ArrayList<>();
